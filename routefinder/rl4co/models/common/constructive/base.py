...
class ConstructiveDecoder(nn.Module, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def forward(
        self, td: TensorDict, hidden: Any = None, num_starts: int = 0,step=None
    ) -> Tuple[Tensor, Tensor]:
...
class ConstructivePolicy(nn.Module):
    ...
    def forward(
        ...
        **decoding_kwargs,
        ...
        pheromone=decoding_kwargs.pop('pheromone',None)
        ...
        while not td["done"].all():
            logits, mask = self.decoder(td, hidden, num_starts,step)
            if pheromone is not None:
                if len(pheromone.size())==3:
                    logits = unbatchify(logits, (8, num_starts))
                    current= unbatchify(td['current_node'], (8, num_starts)).unsqueeze(-1).repeat(1,1,1,num_starts+1)
                    pheromone_temp=pheromone.unsqueeze(0).repeat(logits.shape[0],1,1,1)
                    logits*=pheromone_temp.gather(-2,current)
                    logits = logits.transpose(0,2).reshape(current.shape[0]*8*num_starts,num_starts+1)
                # pheromone
                else:
                    # no expert
                    # logits=1*pheromone[td['current_node']]
                    # FM expert
                    logits=logits*pheromone[td['current_node']]
            # else:
            #     logits=logits*pheromone[td['current_node']] if pheromone is not None else logits
        ...
        if actions_best is not None:
            indices = torch.arange(len(actions_best) - 1)
            mapping = {actions_best[i].item(): actions_best[i + 1].item() for i in indices}

            action_flat = actions.flatten()
            action_replaced = torch.tensor([mapping.get(val.item(), val.item()) for val in action_flat])

            label_best_lkh = action_replaced.view(actions.shape)
        else:
            label_best_lkh=actions
        outdict = {
            "reward": td["reward"],
            "log_likelihood": get_log_likelihood(
                logprobs, actions, td.get("mask", None), return_sum_log_likelihood
            ),
            "KL": get_log_likelihood(
                logprobs, label_best_lkh, td.get("mask", None), return_sum_log_likelihood
            ),
        }
