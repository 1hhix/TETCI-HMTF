...
class MTVRPEnv(RL4COEnvBase):
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        distance[prev_node>curr_node]*=(1-0.5*td['asymmetric_route'][prev_node>curr_node])
    ...
    def _reset(
        ...
        # Capacity (C) route. Defaults to 1
        capacity_route = td.get(
            "capacity_route", torch.ones_like(demand_linehaul[..., :1], dtype=torch.bool)
        )
        # PLM task embedding. Defaults to 0
        task_embedding = td.get(
            "task_embedding", torch.zeros_like(demand_linehaul[..., :1], dtype=torch.bool)
        )
        #  asymmetric_route (A)
        asymmetric_route = td.get(
            "asymmetric_route", torch.zeros_like(demand_linehaul[..., :1], dtype=torch.bool)
        )
    ...
    def get_action_mask(td: TensorDict) -> torch.Tensor:
        ...
        asymmetric_index = torch.arange(d_ij.shape[1],device=td.device) < curr_node.unsqueeze(1)
        d_ij[asymmetric_index]*=(1-0.5*td['asymmetric_route'].expand_as(d_ij)[asymmetric_index])
        d_j0 = get_distance(locs, locs[..., 0:1, :])*(1-0.5*td['asymmetric_route'])  # j (next) -> 0 (depot)
        # Time constraint (TW):
        early_tw, late_tw = (
            td["time_windows"][..., 0],
            td["time_windows"][..., 1],
        ...
        # whether to consider capacity constraint
        meets_demand_constraint=meets_demand_constraint|~td['capacity_route']
        ...
        #  if TSP we set reach depot false
        can_visit[:, 0][(can_visit[:, 1:].sum(-1) > 0)&(~td['capacity_route'].squeeze(1))]=False
    ...
    def _get_reward(self, td: TensorDict, actions: TensorDict) -> TensorDict:
        ...
        #  asymmetric_route
        distances[go_from>go_to]*=(1-0.5*td['asymmetric_route'].expand_as(go_from)[go_from>go_to])
