...
class MTVRPInitEmbeddingRouteFinder(MTVRPInitEmbeddingRouteFinderBase):
    ...
    def __init__(self, embed_dim=128, bias=False, posinf_val=0.0):
        super(MTVRPInitEmbeddingRouteFinder, self).__init__(
            num_global_feats=7,  # x, y, open_route, distance_limit, time_window_depot,capacity,asymmetric
            num_cust_feats=7,
            embed_dim=embed_dim,
            bias=bias,
    ...
    def _global_feats(self, td):
        return torch.cat(
            [
                ...
                td['asymmetric_route'].float()[..., None],
                td['capacity_route'].float()[..., None],
            ],
            -1,
        )
...
class DFM_InitEmbedding(MTVRPInitEmbeddingRouteFinderBase):
    """
    Customer features:
        - locs: x, y euclidean coordinates
        - demand_linehaul: demand of the nodes (delivery) (C)
    ...
    def __init__(self, embed_dim=128, bias=False, posinf_val=0.0):
        super(DFM_InitEmbedding, self).__init__(
            num_global_feats=4,  # x, y, open_route, distance_limit, time_window_depot,capacity,asymmetric
            num_cust_feats=7,
            embed_dim=embed_dim,
            bias=bias,
            posinf_val=posinf_val,
