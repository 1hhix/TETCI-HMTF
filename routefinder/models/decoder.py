...
class AttentionModelDecoder(AutoregressiveDecoder):
    ...
    def _compute_q(self, cached: PrecomputedCache, td: TensorDict,step=None):
...
class TaskEncoder(nn.Module):
    ...
    def forward(self, c,query):
        global_task_feature = self.global_projector(c)
        return [decoder(global_task_feature) for decoder in self.output]

class Query_task(nn.Module):
    def __init__(self, embed_dim=128,decoder_time_step=False):
        super().__init__()
        self.query_norm = RMSNorm(embed_dim)
        self.ffn_norm = RMSNorm(embed_dim)
        self.norm = RMSNorm(embed_dim)
        self.decoder_time_step=decoder_time_step
        if decoder_time_step:
            self.time_mlp=nn.Sequential(
            nn.Linear(7, 2 * embed_dim, bias=True),
            nn.SiLU(),
            nn.Linear(2*embed_dim, 4 * embed_dim, bias=True),
        )
            self.binary = torch.tensor([64, 32, 16, 8, 4, 2, 1], dtype=torch.int32)
        else:
            self.query_shaft = TaskEncoder(embed_dim, 4)
        self.act=nn.SiLU()
        self.att = MultiHeadAttention(
            embed_dim, 8, bias=False, sdpa_fn=None
        )
    def forward(self,query,td,time_step=None):
        if self.decoder_time_step:
            # performance for training is better but generalize is poor
            binary_step=((time_step&self.binary)==0).to(td.device,query.dtype)
            shift_query,scale_query,shift_query_back,scale_query_back=self.time_mlp(binary_step).chunk(4, dim=-1)
            query=query+self.att(self.query_norm(modulate(query, shift_query, scale_query)))
            return query+self.norm(modulate(query, shift_query_back, scale_query_back))
        else:
            if time_step==0:
                shift_query,scale_query,gate_msa,gate_mlp= self.query_shaft(td['task_embedding'],query)
                self.shift_query=shift_query
                self.scale_query=scale_query
                self.gate_msa=gate_msa
                self.gate_mlp=gate_mlp
            query=query+self.att(self.query_norm(modulate(query, self.shift_query, self.scale_query)))
            return  query+ self.norm(modulate(query, self.gate_msa, self.gate_mlp))




class FM_decoder(AutoregressiveDecoder):
    def __init__(
        self,
        embed_dim: int = 128,
        num_heads: int = 8,
        ...
        self.use_hyper_decoder=use_hyper_decoder
        if use_hyper_decoder:
            self.task_query=Query_task(embed_dim=embed_dim,decoder_time_step=decoder_time_step)
    ...
    def _compute_q(self, cached: PrecomputedCache, td: TensorDict,step=None):
        ...
        if self.use_hyper_decoder:
            return self.task_query(glimpse_q,td,time_step=step)
        else:
            return glimpse_q
    ...
    def forward(
        ...
        glimpse_q = self._compute_q(cached, td,step=step)
        glimpse_k, glimpse_v, logit_k = self._compute_kvl(cached, td)
